...
for (const population of populations){
    ...
    const cleaned = separator ? text.split(separator).join('').trim() : text.trim();
...
document.querySelector('.total-population').textContent = totalPopulation.toLocaleString('en-US').replace(/,/g, separator);
...
} else averageEl.textContent = String(0).toLocaleString('en-US').replace(/,/g, separator);
